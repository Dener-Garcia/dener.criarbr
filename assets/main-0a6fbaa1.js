import{A as F,C as y,s as p}from"./scale-text-440c2bea.js";import{S as g}from"./show-menu-7b28359c.js";import{R as C}from"./animation-reveral-6b326b36.js";class w extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
    ...
    `}}const u=()=>{const i=document.querySelector("form"),r=document.querySelector("#fieldName"),e=document.querySelector("#fieldMail"),o=document.querySelector("#fieldMessage"),s=document.querySelector("form button");i.addEventListener("submit",t=>{if(t.preventDefault(),a(r),a(e),a(o),!a(r))return;if(!d(e.value)){h(e,"Ops! E-mail incorreto. Tente algo como: seu_email@email.com.br");return}if(!a(o))return;const n={name:r.value,mail:e.value,message:o.value};fetch("https://apisendmail.criarbr.com/enviar-email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}).then(l=>l.json()).then(l=>{l.message?(m([r,e,o]),console.info("Resposta do server:",l.message)):(c("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Resposta do server:",l.error))}).catch(l=>{c("Falha no envio, tente novamente","ph","ph-warning-diamond","inputInvalidJS"),console.error("Erro ao enviar dados do formulário:",l)})});function a(t){return t.value===""?(h(t,"Ops! Você esqueceu de preencher esse campo"),!1):(h(t,""),t.classList.remove("inputInvalidJS"),t.classList.add("inputValidJS"),!0)}function d(t){return!!new RegExp(/^[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,}$/).test(t)}function h(t,n){const k=t.parentElement.querySelector("legend");k.textContent=n,t.classList.remove("inputValidJS"),t.classList.add("inputInvalidJS")}function m(t){setTimeout(()=>{t.forEach(n=>{n.classList.remove("inputValidJS"),n.value=""}),c("Mensagem enviada","ph","ph-check-fat","inputValidJS")},1500)}function c(t,n,l,k){s.textContent=t,s.classList.add(k);const f=document.createElement("i");f.classList.add(n),f.classList.add(l),s.appendChild(f)}},L=()=>{document.querySelectorAll(".certificate-box").forEach(r=>{r.addEventListener("mouseenter",()=>{const e=r.querySelector("details");e.hasAttribute("open")?e.removeAttribute("open"):e.setAttribute("open","")})})},x=async()=>{try{const r=await(await fetch("/card-projects.json")).json(),e=document.querySelector(".grid-projects");r.contentCard.map(o=>{const s=o,a=document.createElement("div");a.classList.add("card-project");const d=document.createElement("img");d.src=s.imgLink,d.alt=s["alt-img"],a.appendChild(d);const h=document.createElement("h3");h.textContent=s.title,a.appendChild(h);const m=document.createElement("p");m.textContent=s.description,document.documentElement.lang==="en"&&(m.textContent=s.descriptionEn),a.appendChild(m);const c=document.createElement("a");c.classList.add("btn-primary"),c.textContent="Ver Projeto",document.documentElement.lang==="en"&&(c.textContent="See Project"),c.href=s.linkPage,document.documentElement.lang==="en"&&(c.href=s.linkPageEn),c.setAttribute("data",s.name),a.appendChild(c),e.appendChild(a)})}catch(i){console.error("Erro na requisição:",i)}},v=()=>{document.querySelector(".grid-projects").addEventListener("pointerover",()=>{document.querySelectorAll(".card-project a").forEach(e=>{let o=e.getAttribute("data");e.addEventListener("click",()=>{localStorage.setItem("name-link",o)})})})},M=(i,r)=>{const e=i;document.addEventListener("scroll",()=>{e.forEach(o=>{let s=window.innerHeight;o.getBoundingClientRect().top<s-r?o.classList.remove("reveral-left"):o.classList.add("reveral-left")})})},E=(i,r)=>{const e=i;document.addEventListener("scroll",()=>{e.forEach(o=>{let s=window.innerHeight;o.getBoundingClientRect().top<s-r?o.classList.remove("reveral-right"):o.classList.add("reveral-right")})})};window.addEventListener("load",()=>{window.customElements.define("my-svg",w)});F();y();p();g();u();L();x();v();const S=document.querySelectorAll(".certificate-box");C(S,-100);const b=document.querySelectorAll(".left");M(b,50);const R=document.querySelectorAll(".right");E(R,50);
